...
const Toolbox = () => {
    ...
    const showStrokeToolOption = actionMenuItem === MENU_ITEMS.PENCIL;
    const showBrushToolOption =  actionMenuItem === MENU_ITEMS.PENCIL || actionMenuItem === MENU_ITEMS.ERASER;
    const { color, size } = useSelector((state)=> state.toolbox[actionMenuItem]);
    const updateBrushSize = (e) => {
        dispatch(changeBrushSize({item: actionMenuItem, size: e.target.value }));
        socket.emit('changeConfig',{color, size: e.target.value});
    }
    
    const updateColor = (newColor) => {
        dispatch(changeColor({item: actionMenuItem, color: newColor }))
        ...
        <div className={styles.toolboxContainer}>
           {showStrokeToolOption && <div className={styles.toolItem}>
            ...
            {showBrushToolOption && <div className={styles.toolItem}>
                <h4 className={styles.toolText}>Brush Size {actionMenuItem}</h4>
                <div className={styles.itemContainer}>
